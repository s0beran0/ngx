...
These rules come from actual defects in this codebase, not from theory. Each
one notes the cost it avoids.
...
**No failures are hidden** — verified by breaking a test on purpose, because
the alternative would be a silent failure no test could reveal: RTK shows the
name, file, line and message, and reports the path of the complete output, which
it saves to disk.

- `go test` is rewritten automatically. The whole suite reports
  `go test: 160 passed in 5 packages` instead of 16 KB of output.
- **`Read`, `Grep` and `Glob` from Claude Code do not pass through the hook.**
  Since a good part of a subagent's consumption is file reading, the real gain
  here is lower than the advertised 60–90%.
- **`git` is excluded from interception**, on purpose. The review flow builds
  packets with `git diff a..b > file`, and the proxy would collapse the diff to
  `--stat`, leaving the reviewer with filenames and no content. The exclusion
  lives in `~/Library/Application Support/rtk/config.toml`.

It helps with `go test`, not with everything.
...
For every token the model wrote, 243 were resent as history. Each turn rereads
all the accumulated context, and that context grows with every turn — so the
cost does not grow with the number of turns, it grows with the **square** of it.
One agent of 232 turns cost 44 million; each of two at 104 cost 11 million.
...
**What is NOT the lever:** compressing tool output. All the combined tool
results — Read, Bash, Edit, Write — come to ~245 thousand tokens, or 0.08% of
the reread history. Output compression tools attack that slice. They are not
useless, but they are not the lever.
...
What inflamed it: **the subagents' reports come back whole.** A review that
returns 3 thousand tokens of analysis, received 300 turns before the end, costs
~900 thousand tokens on its own, because it is reread on every subsequent turn.

This goes against Anthropic's guidance on subagents, which is to return
**condensed summaries of 1,000 to 2,000 tokens** after exploring extensively.
