...
//
// Neither failure is an ngx error. Both come wrapped in errNoSSHAgent and
...
func connectSSHAgent() (net.Conn, error) {
	socketPath := os.Getenv(EnvSocketSSHAgent)
	if socketPath == "" {
		return nil, fmt.Errorf("%w: %s is not set in the environment", errNoSSHAgent, EnvSocketSSHAgent)
	}

	conn, err := net.Dial("unix", socketPath)
	if err != nil {
		return nil, fmt.Errorf("%w: could not talk to the socket %s: %w",
			errNoSSHAgent, socketPath, err)
	}
	return conn, nil
}
